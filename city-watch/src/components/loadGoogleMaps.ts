...
let googleMapsLoadedLibraries = new Set<string>()

function loadScript(src: string) {
  return new Promise<void>((resolve, reject) => {
    // Avoid double-loading the Google Maps JS API.
    const anyExisting = document.querySelector<HTMLScriptElement>(
      'script[src^="https://maps.googleapis.com/maps/api/js"]',
    )
    if (anyExisting) {
      const alreadyLoaded =
        anyExisting.dataset.loaded === 'true' || Boolean((window as any).google?.maps)
...
export function loadGoogleMaps(
  ...
  if (!apiKey) {
    return Promise.reject(
      new Error(
        'Missing Google Maps API key. Set VITE_GOOGLE_MAPS_API_KEY in a .env file.',
      ),
    )
  }
  ...
  // If Maps is already present, just validate requested libraries.
  if ((window as any).google?.maps) {
    const missing = requestedLibs.filter((l) => !googleMapsLoadedLibraries.has(l))
    if (missing.includes('places') && !(window as any).google?.maps?.places) {
      return Promise.reject(
        new Error(
